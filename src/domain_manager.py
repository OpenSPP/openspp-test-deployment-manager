...
class DomainManager:
    ...
    def generate_nginx_config(self, deployment: Deployment) -> str:
        ...
        # Support both external (test.openspp.org) and internal (.internal) domains
        external_domain = deployment.subdomain  # e.g., test1.test.openspp.org
        internal_domain = f"{deployment.id}.internal"  # e.g., test1.internal
...
server {{
    listen 80;
    server_name mailhog-{external_domain} mailhog-{internal_domain};
...
server {{
    listen 80;
    server_name pgweb-{external_domain} pgweb-{internal_domain};
